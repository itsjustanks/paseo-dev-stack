...
if [ "$(id -u)" = "0" ]; then
  ...
  # `docker exec` without `--user paseo` runs the agent CLIs (and npm) as root, leaving
  # root-owned files *inside* otherwise paseo-owned dirs. Codex then warns "failed to clean
  # up stale arg0 temp dirs: Permission denied" (~/.codex/tmp/arg0), and npm-based plugin
  # installs fail with EEXIST on ~/.npm/_cacache. Reclaim just those files; paseo-owned ones
  # are left untouched.
  for d in "$HOME_DIR/.codex" "$HOME_DIR/.claude" \
           "$HOME_DIR/.npm" "$HOME_DIR/.cache" "$HOME_DIR/.local"; do
